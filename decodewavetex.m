function eodtext=decodewavetex(wave_text, version)
%DECODEWAVETEX
%
...
eodtext.lowpass=' ';
eodtext.highpass=' ';
eodtext.conductivity=' ';
eodtext.voltagedivider=' ';
eodtext.eodcalibrated='False';
eodtext.eodcalibrationratio='NA';
eodtext.eodcalibrationdistance='NA';

switch version
    case 2
         tdata=strsplit(wave_text,';');
         for i=1:length(tdata)
            metachunk=strtrim(strsplit(tdata{i},'='));
            structcommand=["eodtext.",lower(metachunk{1}),"='",metachunk{2},"';"];
            eval(strjoin(structcommand,''))
         end
